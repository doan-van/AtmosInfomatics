...
def test_pnp(precips_mm_monthly,
    ...
    np.testing.assert_allclose(computed_pnp.flatten(),
                               all_nan_precips.flatten(),
                               equal_nan=True,
                               err_msg="All-NaN input array does not result "
                                       "in the expected all-NaN result")

    # compute PNP from the daily precipitation array
    computed_pnp_6month = indices.percentage_of_normal(precips_mm_monthly.flatten(),
                                                       6,
                                                       data_year_start_monthly,
                                                       calibration_year_start_monthly,
                                                       calibration_year_end_monthly,
    ...
    np.testing.assert_allclose(pnp_6month.flatten(),
    ...
    indices.percentage_of_normal(precips_mm_daily.flatten(),
                                 30,
                                 data_year_start_daily,
                                 calibration_year_start_daily,
                                 calibration_year_end_daily,
...
def test_spi(precips_mm_monthly,
    ...
    indices.spi(precips_mm_daily.flatten(),
                60,
...
def test_spei(precips_mm_monthly,
    ...
    np.testing.assert_allclose(computed_spei,
                               all_nans,
                               equal_nan=True,
                               err_msg="SPEI/Gamma not handling all-NaN "
                                       "arrays as expected")

    # compute SPEI/gamma at 6-month scale
    computed_spei = indices.spei(precips_mm_monthly,
                                 pet_thornthwaite_mm,
                                 6,
                                 indices.Distribution.gamma,
                                 compute.Periodicity.monthly,
    ...
    np.testing.assert_allclose(computed_spei,
                               spei_6_month_gamma,
                               atol=0.01,
                               err_msg="SPEI/Gamma values for 6-month "
                                       "scale not computed as expected")
    ...
    np.testing.assert_allclose(computed_spei,
                               spei_6_month_pearson3,
                               atol=0.01,
                               err_msg="SPEI/Pearson values for 6-month scale "
                                       "not computed as expected")
